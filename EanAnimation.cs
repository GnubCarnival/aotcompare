...
public class EanAnimation
{
  ...
  public void Load(BinaryReader br, FileStream fs)
  {
    ...
    for (int index = 0; index < (int) this.TotalCount; ++index)
    {
      this.Frames[index].X = br.ReadUInt16();
      this.Frames[index].Y = br.ReadUInt16();
      this.Frames[index].Width = br.ReadUInt16();
      this.Frames[index].Height = br.ReadUInt16();
    }
