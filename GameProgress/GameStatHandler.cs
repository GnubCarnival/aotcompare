  ...
  internal class GameStatHandler : BaseGameProgressHandler
  {
    ...
    public GameStatHandler(GameStatContainer gameStat)
    {
      ...
      for (int index = 1; index < 20; ++index)
        this._expPerLevel.Add((int) ((double) this._expPerLevel[index - 1] * 1.2000000476837158));
    }

    public int GetExpToNext() => this._gameStat.Level.Value >= 20 ? 0 : this._expPerLevel[this._gameStat.Level.Value];
    ...
    private void CheckLevelUp()
    {
      ...
      {
        if (this._gameStat.Exp.Value <= 0 || this._gameStat.Exp.Value < this._expPerLevel[this._gameStat.Level.Value])
          return;
    ...
    public override void RegisterSpeed(GameObject character, float speed) => this._gameStat.HighestSpeed.Value = Mathf.Max(this._gameStat.HighestSpeed.Value, speed);
